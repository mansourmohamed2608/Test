...
const getImages = async(req, res) => {
    try {
        const [rows, fields] = await connection.query('SELECT * FROM community')
...
const addNewImage = async (req, res) => {
    ...
    try {
        const [rows, fields] = await connection.query('INSERT INTO community SET ?', {img: req.body.url, imglabel: req.body.label})
...
const removeImage = async(req, res) => {
    ...
    try {
        const [rows, fields] = await connection.query('DELETE FROM community WHERE id = ?', [req.body.imgid])
        if (rows.affectedRows > 0) {
